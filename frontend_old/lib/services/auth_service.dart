  ...
  Future<Map<String, dynamic>> me() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return {'user': null, 'status': null, 'detail': null};
      }
    } catch (e) {
  ...
  Future<http.Response> authenticatedRequest(Future<http.Response> Function() request) async {
    try {
      if (!(await tokenExists())) {
        return http.Response(json.encode({'detail': 'Unauthorized'}), 401);
      }
      var response = await request();
      return response;
    } catch (e) {
      return http.Response(json.encode({'detail': 'Network error: ${e.toString()}'}), 404);
    }
  }
}
